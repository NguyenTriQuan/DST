...
def NPB_linear_forward(self, x):
    if self.measure:
        ...
        x_max = torch.max(x)
        return torch.log(F.linear((x-x_max).exp(), self.mask, None)) + x_max
    else:
        if self.training:
...
def NPB_conv_forward(self, x):
    if self.measure:
        ...
        # return nodes_out, paths_out
        x_max = torch.max(x)
        return torch.log(self._conv_forward((x-x_max).exp(), self.mask, None)) + x_max
    else:
        if self.training:
            self.mask = TopK.apply(self.score.abs(), self.num_zeros)
...
def NPB_stable_forward(self, x):
    if self.measure:
        x_max = torch.max(x)
        return torch.log(self.original_forward((x-x_max).exp())) + x_max
    else:
        return self.original_forward(x)
...
def NPB_register(model):
    ...
    for m in model.modules():
        ...
        elif isinstance(m, nn.MaxPool2d) or isinstance(m, nn.AvgPool2d) or isinstance(m, nn.Flatten) or isinstance(m, nn.Dropout):
            setattr(m, 'original_forward', m.forward)
            setattr(m, 'forward', NPB_stable_forward.__get__(m, m.__class__))
        elif isinstance(m, nn.BatchNorm2d) or isinstance(m, nn.LogSoftmax) or isinstance(m, nn.ReLU):
            setattr(m, 'original_forward', m.forward)
