...
def NPB_linear_forward(self, x):
    if self.training:
        ...
        eff_paths = torch.log(F.linear((eff_paths - max_paths).exp(), self.mask, None)) + max_paths
...
def NPB_conv_forward(self, x):
    if self.training:
        ...
        eff_paths, images = x
        max_paths = eff_paths.max()
        eff_paths = torch.log(self._conv_forward((eff_paths - max_paths).exp(), self.mask, None)) + max_paths
